...
完成技术检查后，逐项过以下**设计质量清单**：

| 项目 | 检查标准 |
|------|---------|
| 强调色一致 | 全文只使用该主题的一种强调色，无随意补色 |
| 字号层级 | 全文最多 4 个字号（H1 / H2 / H3 / 正文），无多余层级 |
| 行高 | 中文正文 `line-height` ≥ `1.7` |
| 无破折号装饰 | 正文段落不用 `—` 作装饰性标点 |
| 无紫色渐变 | 背景/分隔线不出现紫色或紫色渐变 |
| 无三等宽卡片 | 不出现 3 个宽度相同的并排信息卡（代码已限制为最多 2 张）|
| 无衬线体滥用 | 仅 Nature 主题可使用衬线正文，其余主题衬线体只出现在标题 |
| 留白节奏 | 各 H2 章节前后间距一致，不出现局部拥挤 |
...
## 设计质量规范（基于 Taste-Skill 原则）

本节规则适用于所有主题，是生成 HTML 时的底层约束，优先级高于风格偏好。

### 禁止事项（Anti-AI Tells）

以下模式会让文章显得由 AI 模板生成，必须避免：

| 禁止项 | 说明 |
|--------|------|
| 紫色渐变 | 背景、分隔线、色块禁止使用紫色（`#8b5cf6`、`#7c3aed` 等）或紫色渐变 |
| 三等宽卡片 | 3 个完全相同宽度的并排 `div` 是最强烈的 AI 模板信号，禁止出现 |
| 破折号装饰 | 禁止在正文中以 `—` 作为视觉装饰（引用来源除外）|
| 通用科技渐变头图 | 禁止蓝紫渐变 + 白色大字的通用科技封面风格 |
| 多余强调色 | 每个主题只允许一种强调色，禁止随意增加第二种彩色 |
| 图标堆砌 | 禁止大量 emoji 或 Unicode 图标代替真实数据或图表 |

### 字体与排版约束

```
字号层级：H1(22-28px) → H2(16-18px) → H3(14-15px) → 正文(14-15px)
字号总数：全文最多使用 4 个不同字号
行高：中文正文 line-height 不低于 1.7，标题区 1.3-1.4
字重：粗体（font-weight:700+）只用于标题和关键数据，正文禁止加粗整段
字体：每个主题最多引用 2 个字体族（衬线 + 无衬线），禁止混搭第三种
```

### 色彩纪律

```
强调色：全文锁定为该主题定义的唯一强调色（如橙皮书 #e85d04）
背景色：最多 2 层（页面底色 + 卡片底色），禁止多层嵌套不同底色
灰度：辅助信息使用主题灰度系统（#999/#666/#333），不引入额外灰色值
对比度：正文文字与背景对比度不低于 4.5:1（WCAG AA）
```

### 间距节奏

```
章节间距：H2 前 margin-top ≥ 32px，保持全文一致
段落间距：正文段落 margin-bottom 统一为 1em（约 14-15px）
内容区内边距：左右 padding 统一为 28-32px，全文不混用
避免孤立元素：图片/引用框不悬浮在段落中间，前后各留段落间距
```

### 内容结构约束

```
CTA 唯一性：全文只允许一个"关注 / 订阅 / 更多"引导，不重复
摘要框：每篇文章最多一个摘要/导读框
眉批标签（eyebrow）：每三个 H2 章节最多出现一次系列标签
信息卡数量：统计数据卡片最多 2 个（禁止 3 等宽并排），超出时改用列表
```

### 设计读取原则

在生成 HTML 之前，先用一句话描述对文章的设计判断：

> 「本文是 [文章类型] 面向 [受众]，调性偏 [风格]，采用 [主题名] 排版。」

这一步确保选择的主题和密度设置与内容匹配，而不是机械套用默认模板。
